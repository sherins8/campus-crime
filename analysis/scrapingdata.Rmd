...
may2022 <- unccrime %>%
  ...
  group_by(event_start_date) %>%
  summarize(number_of_crimes = n()) %>%
  mutate(event_start_date = format(event_start_date, "%m-%d")) %>%
  clean_names()
...
maycrime <- ggplot() +
  geom_line(data = may2022, aes(x = event_start_date, y = number_of_crimes, color = "blue")) +
  geom_line(data = may2023, aes(x = event_start_date, y = number_of_crimes, color = "green")) +
  labs(x = "Date", y = "# of Crimes", title = "Chapel Hill May Crime Trends") +
...
gap_in_data <- unccrime %>%
  mutate(gap_days = as.numeric(difftime(date_reported, event_end_date, units = "days")),
...
time_diff <- unccrime %>%
  mutate(time_between = as.numeric(difftime(date_reported, event_end_date, units = "days"))) %>%
  pull(time_between)
